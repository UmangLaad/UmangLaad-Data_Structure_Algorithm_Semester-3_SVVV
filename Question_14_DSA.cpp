// Q: Infix to Postfix considering Parenthesis and Associativity using Stack.
...
// Function to return precedence for operators inside the stack [Umang Laad]
    ...
    while (infix[i] != '\0')
    {
        ...
        else
        {
            if (OutStackPre(infix[i]) > InStackPre(top->data)) //if the precedence of the character outside the stack is greater than the precedence of the character inside the stack
                push(infix[i++]);                              //push the character from outside to inside of the stack  //point to be noted: logically after pushing the character inside the stack its precedence increases or decreases according to its associativity
            else
            {
